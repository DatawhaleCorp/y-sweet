...
use rusty_oss::{Bucket, Credentials, OSSAction};
...
pub struct OssConfig {
    ...
    // no matter what, we always use virtual host style
    pub path_style: bool,
}
...
impl OssStore {
    pub fn new(config: OssConfig) -> Self {
        ...
        let path_style = if config.path_style {
            // We only support VirtualHost URLs for OSS-compatible APIs
            rusty_oss::UrlStyle::VirtualHost
        } else {
            rusty_oss::UrlStyle::VirtualHost
        };
    ...
    async fn store_request<'a, A: OSSAction<'a>>(
        &self,
        method: Method,
        action: A,
        body: Option<Vec<u8>>,
        ...
        match response.status() {
            ...
            StatusCode::NOT_FOUND => Err(StoreError::DoesNotExist(
                "Received NOT_FOUND from OSS-compatible API.".to_string(),
            )),
            StatusCode::FORBIDDEN => Err(StoreError::NotAuthorized(
                "Received FORBIDDEN from OSS-compatible API.".to_string(),
            )),
            StatusCode::UNAUTHORIZED => Err(StoreError::NotAuthorized(
                "Received UNAUTHORIZED from OSS-compatible API.".to_string(),
            )),
            _ => Err(StoreError::ConnectionError(format!(
                "Received {} from OSS-compatible API.",
                response.status()
            ))),
        }
    }
    ...
    pub async fn init(&self) -> Result<()> {
        ...
        let action = self.bucket.get_bucket_info(Some(&self.credentials));
